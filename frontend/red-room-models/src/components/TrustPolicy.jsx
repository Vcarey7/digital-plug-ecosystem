...
const SECTIONS = [
  {
    ...
    body: "Every model is either fully synthetic or backed by a documented, signed model release for any real-person likeness used in training. Creators attest to this on every listing, and we can request the underlying documentation at any time.",
  },
  {
    icon: Ban,
  ...
  },
  {
    ...
    body: "Every submission passes a human curatorial and compliance review — checking rights documentation, content standards, and platform-policy fit — before it becomes visible in the Red Room.",
  },
  {
    icon: ShieldCheck,
    title: "Age-gated platform",
    body: "Red Room Models is an 18+ platform for both creators and buyers. Age assurance is required at account creation, and access is revoked if misrepresented.",
  },
];
...
export default function TrustPolicy() {
  return (
    <div className="mx-auto max-w-4xl px-6 py-16 lg:px-10">
      <p className="text-xs uppercase tracking-widest2 text-crimson">Trust & Compliance</p>
      <h1 className="mt-2 font-display text-4xl italic text-ivory">
        The Red Room Models standard
      </h1>
      <p className="mt-4 text-ivory/60">
        Licensing an AI persona is not the same as licensing a stock photo. We
        built these standards because the models on this platform are meant for
      ...
      <div className="mt-12 flex flex-col gap-8">
        {SECTIONS.map((s) => (
          <div key={s.title} className="flex gap-4 border-b border-white/10 pb-8">
            <s.icon size={22} className="mt-1 shrink-0 text-scarlet" strokeWidth={1.5} />
            <div>
      ...
      <div className="mt-10 rounded-sm border border-scarlet/20 bg-charcoal p-6 text-sm text-ivory/50">
